class SuperDistraction < Ability
  ...
  def update_defender(defender, mode = :pvp)
    defender.targets('highest_dmg').each {|target| target.remove_dodge}
  end
  ...
  def update_defender_after_damage(defender, mode = :pvp)
    defender.targets('all_opponents').each {|target| target.add_modifier(Modifiers::Distraction.new(50, 1, 2))}
    defender.targets('highest_dmg').each {|target| target.add_modifier(Modifiers::Distraction.new(50, 1, 2))}
    defender.targets('highest_dmg').each {|target| target.add_modifier(Modifiers::DamageOverTime.new(25, 2))}
  end
  ...
  def damage_defender(attacker, defender, mode = :pvp)
    result = super
    defender.targets('highest_dmg').each {|target| target.is_stunned = rand(100) < 25.0 * (100.0 - target.resistance(:stun)) / 100.0}
    result
  end
