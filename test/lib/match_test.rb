...
class MatchTest < ActiveSupport::TestCase
  ...
  test "Dino with counter should win" do
    d1 = Dinosaur.new(health: 1000, damage: 450, speed: 130, level: 20, name: 'd1', abilities: [Strike], abilities_counter: [MediumCounterAttack], strategy: DefaultStrategy)
    d2 = Dinosaur.new(health: 1000, damage: 450, speed: 130, level: 20, name: 'd2', abilities: [Strike], strategy: DefaultStrategy)
    match = Match.new(d1, d2)
    result = match.execute
    assert_equal "d1", result[:outcome]
  end
