class GroupAcceleration < Ability
  ...
  def update_attacker(attacker, mode = :pvp)
    attacker.targets('team').each {|target| target.add_modifier(Modifiers::IncreaseSpeed.new(50, 3, nil))}
    attacker.targets('team').each {|target| target.remove_speed_decrease}
  end
