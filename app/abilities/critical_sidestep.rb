class CriticalSidestep < Ability
  ...
  def update_attacker(attacker, mode = :pvp)
    attacker.targets('self').each {|target| target.add_modifier(Modifiers::IncreaseCriticalChance.new(100, 2, 1))}
    attacker.targets('self').each {|target| target.add_modifier(Modifiers::Dodge.new(100, 0, 2))}
