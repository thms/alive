class AlertRoost < Ability
  ...
  def update_attacker(attacker, mode = :pvp)
    attacker.targets('self').each {|target| target.add_modifier(Modifiers::IncreaseCriticalChance.new(100, 2, 2))}
    attacker.targets('self').each {|target| target.add_modifier(Modifiers::Dodge.new(75, 2, 2))}
  end
