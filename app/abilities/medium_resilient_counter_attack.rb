class MediumResilientCounterAttack < Ability
  ...
  def update_defender(defender, mode = :pvp)
    defender.targets('attacker').each {|target| target.remove_speed_increase}
    defender.targets('attacker').each {|target| target.remove_cloak}
    defender.targets('attacker').each {|target| target.remove_dodge}
  end
  ...
  def update_defender_after_damage(defender, mode = :pvp)
    defender.targets('attacker').each {|target| target.add_modifier(Modifiers::Vulnerability.new(50, 2, 1))}
  end
