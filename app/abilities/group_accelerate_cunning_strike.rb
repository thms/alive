class GroupAccelerateCunningStrike < Ability
  ...
  def update_defender(defender, mode = :pvp)
    defender.targets('lowest_hp').each {|target| target.remove_critical_chance_increase}
    defender.targets('lowest_hp').each {|target| target.remove_attack_increase}
  end
  ...
  def update_defender_after_damage(defender, mode = :pvp)
    defender.targets('lowest_hp').each {|target| target.add_modifier(Modifiers::Distraction.new(50, 1, 2))}
    defender.targets('lowest_hp').each {|target| target.add_modifier(Modifiers::ReduceCriticalChance.new(100, 1, 2))}
  end
