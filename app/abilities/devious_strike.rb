class DeviousStrike < Ability
  ...
  def update_attacker(attacker, mode = :pvp)
    attacker.targets('self').each {|target| target.add_modifier(Modifiers::IncreaseCriticalChance.new(30, 2, 2))}
  ...
  def update_defender_after_damage(defender, mode = :pvp)
    defender.targets('lowest_hp').each {|target| target.add_modifier(Modifiers::Distraction.new(25, 2, 4))}
  end
