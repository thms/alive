class FerociousDefense < Ability
  ...
  def update_attacker(attacker)
    attacker.zelf.each {|target| target.add_modifier(Modifiers::Shields.new(50, 2, 4))}
  end
  ...
  def update_defender_after_damage(defender)
    defender.zelf.each {|target| target.add_modifier(Modifiers::IncreaseDamage.new(50, 3, 6))}
  end
