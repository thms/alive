...
class SimulationTest < ActiveSupport::TestCase
  ...
  test "Counter attacker should win" do
    d1 = Dinosaur.new(health: 1000, damage: 450, speed: 130, level: 20, name: 'd1', abilities: [Strike], abilities_counter: [MediumCounterAttack], strategy: DefaultStrategy).reset_attributes!
    d2 = Dinosaur.new(health: 1000, damage: 450, speed: 130, level: 20, name: 'd2', abilities: [Strike], strategy: DefaultStrategy).reset_attributes!
    simulation = Simulation.new(d1, d2)
    result = simulation.execute
    wins_and_losses = simulation.calc_wins_and_losses(result)
