class VenomousCounter < Ability
  ...
  def update_defender_after_damage(defender, mode = :pvp)
    defender.targets('attacker').each {|target| target.add_modifier(Modifiers::Distraction.new(50, 1, nil))}
    defender.targets('attacker').each {|target| target.add_modifier(Modifiers::PreventSwap.new(1, 'other'))}
  end
