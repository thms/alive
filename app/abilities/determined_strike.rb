class DeterminedStrike < Ability
  ...
  def update_defender(defender)
    defender.fastest.each {|target| target.remove_critical_chance_increase}
    defender.fastest.each {|target| target.remove_attack_increase}
  ...
  def update_defender_after_damage(defender)
    ...
    defender.fastest.each {|target| target.add_modifier(Modifiers::Distraction.new(50, 1, 2))}
    defender.fastest.each {|target| target.add_modifier(Modifiers::ReduceCriticalChance.new(100, 1, 2))}
  end
