class SlipperyAlert < Ability
  ...
  def update_attacker(attacker, mode = :pvp)
    ...
    attacker.targets('self').each {|target| target.add_modifier(Modifiers::Dodge.new(50, 2, 2))}
  ...
  def update_defender_after_damage(defender, mode = :pvp)
    defender.targets('highest_dmg').each {|target| target.add_modifier(Modifiers::Distraction.new(50, 2, 2))}
  end
