class InstantBuff < Ability
  ...
  def update_attacker(attacker, mode = :pvp)
    attacker.targets('self').each {|target| target.add_modifier(Modifiers::IncreaseDamage.new(50, 2, 1))}
    attacker.targets('self').each {|target| target.add_modifier(Modifiers::Dodge.new(75, 0, 2))}
