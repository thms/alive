class GroupShatteringImpact < Ability
  ...
  def update_defender(defender, mode = :pvp)
    defender.targets('all_opponents').each {|target| target.destroy_shields}
    defender.targets('all_opponents').each {|target| target.remove_taunt}
  end
