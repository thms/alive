class RevengeCloak < Ability
  ...
  def update_attacker(attacker)
    attacker.zelf.each {|target| target.add_modifier(Modifiers::Cloak.new(75, 100, 2, nil))}
  end

  # same as above but called when the attacker is in revenge mode
  def update_attacker_revenge(attacker)
    attacker.zelf.each {|target| target.add_modifier(Modifiers::Cloak.new(75, 200, 1, nil))}
    self.delay = 0
    self.cooldown = 1
  end
