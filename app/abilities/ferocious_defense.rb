class FerociousDefense < Ability
  ...
  def update_attacker(attacker, mode = :pvp)
    attacker.targets('self').each {|target| target.add_modifier(Modifiers::Shields.new(50, 2, 4))}
    attacker.targets('self').each {|target| target.add_modifier(Modifiers::IncreaseDamage.new(50, 3, 6))}
  end
