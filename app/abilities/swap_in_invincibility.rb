class SwapInInvincibility < Ability
  ...
  def update_attacker(attacker, mode = :pvp)
    ...
    attacker.targets('self').each {|target| target.add_modifier(Modifiers::Taunt.new(0, nil))}
    attacker.targets('self').each {|target| target.add_modifier(Modifiers::Shields.new(100, 0, 2))}
  end
