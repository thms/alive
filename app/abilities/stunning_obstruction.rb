class StunningObstruction < Ability
  ...
  def update_defender_after_damage(defender, mode = :pvp)
    defender.targets('escapee').each {|target| target.add_modifier(Modifiers::PreventSwap.new(2, 'other'))}
  end
  ...
  def damage_defender(attacker, defender, mode = :pvp)
    result = super
    defender.targets('escapee').each {|target| target.is_stunned = rand(100) < 100 * (100.0 - target.resistance(:stun)) / 100.0}
    result
  end
