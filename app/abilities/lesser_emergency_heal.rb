class LesserEmergencyHeal < Ability
  ...
  self.attacker_team_targets = 'lowest_hp_teammate'
  self.defender_team_targets = nil

  # add and remove modifiers for the attacker
  def update_attacker(attacker, mode = :pvp)
    attacker.targets('lowest_hp_teammate').each {|target| target.heal(1 * attacker.damage)}
    attacker.targets('lowest_hp_teammate').each {|target| target.cleanse(:all)}
  end
