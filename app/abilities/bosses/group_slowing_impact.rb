class Bosses::GroupSlowingImpact < Ability
  ...
  def update_defender_after_damage(defender)
    defender.all_opponents.each {|target| target.add_modifier(Modifiers::Vulnerability.new(50, 2, 1))}
    defender.all_opponents.each {|target| target.add_modifier(Modifiers::DecreaseSpeed.new(50, 2, nil))}
  end
