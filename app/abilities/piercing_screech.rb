class PiercingScreech < Ability
  ...
  def update_attacker(attacker, mode = :pvp)
    attacker.targets('team').each {|target| target.add_modifier(Modifiers::IncreaseCriticalChance.new(20, 2, nil))}
    attacker.targets('team').each {|target| target.add_modifier(Modifiers::IncreaseSpeed.new(10, 1, nil))}
  ...
  def update_defender_after_damage(defender, mode = :pvp)
    defender.targets('all_opponents').each {|target| target.add_modifier(Modifiers::Distraction.new(50, 2, 4))}
  end
