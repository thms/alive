class ReadyToCrush < Ability
  ...
  def update_attacker(attacker, mode = :pvp)
    attacker.targets('team').each {|target| target.add_modifier(Modifiers::IncreaseCriticalChance.new(30, 4, 2))}
    attacker.targets('team').each {|target| target.add_modifier(Modifiers::IncreaseDamage.new(50, 4, 2))}
    attacker.targets('team').each {|target| target.remove_critical_chance_decrease}
