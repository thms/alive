class InstantRumble < Ability
  ...
  def update_attacker(attacker)
    attacker.zelf.each {|target| target.add_modifier(Modifiers::IncreaseCriticalChance.new(20, 2, nil))}
    attacker.zelf.each {|target| target.add_modifier(Modifiers::Shields.new(50, 0, 4))}
  end
  ...
  def update_defender_after_damage(defender)
    ...
    defender.zelf.each {|target| target.add_modifier(Modifiers::IncreaseDamage.new(25, 2, nil))}
  end
