class ShieldedDeceleratingStrike < Ability
  ...
  def update_attacker(attacker, mode = :pvp)
    attacker.targets('self').each {|target| target.add_modifier(Modifiers::Shields.new(50, 1, 4))}
  end
  ...
  def update_defender_after_damage(defender, mode = :pvp)
    defender.targets('lowest_hp').each {|target| target.add_modifier(Modifiers::DecreaseSpeed.new(50, 2, nil))}
  end
