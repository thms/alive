class AlertBuffAndStrike < Ability
  ...
  def update_attacker(attacker, mode = :pvp)
    attacker.targets('self').each {|target| target.add_modifier(Modifiers::IncreaseCriticalChance.new(25, 3, 2))}
    attacker.targets('self').each {|target| target.remove_critical_chance_decrease}
    attacker.targets('self').each {|target| target.remove_speed_decrease}
  end
