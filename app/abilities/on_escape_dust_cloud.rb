class OnEscapeDustCloud < Ability
  ...
  def update_attacker(attacker, mode = :pvp)
    attacker.targets('self').each {|target| target.add_modifier(Modifiers::Shields.new(100, 1, 2))}
    attacker.targets('self').each {|target| target.add_modifier(Modifiers::IncreaseDamage.new(50, 1, 2))}
  end
