class EvasiveWoundingStrike < Ability
  ...
  def update_attacker(attacker, mode = :pvp)
    attacker.targets('self').each {|target| target.add_modifier(Modifiers::Dodge.new(50, 2, 4))}
  end
  ...
  def update_defender_after_damage(defender, mode = :pvp)
    defender.targets('highest_dmg').each {|target| target.add_modifier(Modifiers::DamageOverTime.new(20, 2))}
  end
